...
BY_LATITUDE_CSV = REPORTS_DIR / "experimental_campanus_greenwich_2000_2h_by_latitude.csv"
SUMMARY_JSON = REPORTS_DIR / "experimental_campanus_greenwich_2000_2h_summary.json"
CALENDAR_CSV = REPORTS_DIR / "experimental_campanus_greenwich_2000_2h_daily_calendar.csv"
CALENDAR_SUMMARY_JSON = REPORTS_DIR / "experimental_campanus_greenwich_2000_2h_daily_calendar_summary.json"
SLOT_LABELS = tuple(f"{hour:02d}" for hour in range(0, 24, 2))
STATUS_ORDER = (
    ExperimentalCampanusStatus.UNIQUE_ORDERED_SOLUTION,
    ExperimentalCampanusStatus.HORIZON_BRANCH_SELECTION_FAILED,
    ExperimentalCampanusStatus.ASSEMBLY_FAILED,
    ExperimentalCampanusStatus.UNORDERED_CUSP_CYCLE,
)
STATUS_CODE = {
    ExperimentalCampanusStatus.UNIQUE_ORDERED_SOLUTION: "U",
    ExperimentalCampanusStatus.HORIZON_BRANCH_SELECTION_FAILED: "B",
    ExperimentalCampanusStatus.ASSEMBLY_FAILED: "A",
    ExperimentalCampanusStatus.UNORDERED_CUSP_CYCLE: "O",
}
...
def _evaluate(
    latitude: float,
    ts: TimestampMeta,
) -> tuple[ExperimentalCampanusStatus, tuple[float, ...] | None]:
    ...
    with CALENDAR_CSV.open("w", newline="", encoding="utf-8") as calendar_handle:
        ...
        for latitude in latitudes:
            ...
            for ts in timestamps:
                ...
                succeeded = status == ExperimentalCampanusStatus.UNIQUE_ORDERED_SOLUTION
                total_count += 1
