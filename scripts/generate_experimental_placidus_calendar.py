...
CALENDAR_CSV = REPORTS_DIR / "experimental_placidus_greenwich_2000_2h_daily_calendar.csv"
CALENDAR_SUMMARY_JSON = REPORTS_DIR / "experimental_placidus_greenwich_2000_2h_daily_calendar_summary.json"
SLOT_LABELS = tuple(f"{hour:02d}" for hour in range(0, 24, 2))
STATUS_ORDER = (
    ExperimentalPlacidusStatus.UNIQUE_ORDERED_SOLUTION,
    ExperimentalPlacidusStatus.NO_REQUIRED_ROOTS,
    ExperimentalPlacidusStatus.NO_ORDERED_SOLUTION,
    ExperimentalPlacidusStatus.AMBIGUOUS_ORDERED_SOLUTION,
)
STATUS_CODE = {
    ExperimentalPlacidusStatus.UNIQUE_ORDERED_SOLUTION: "U",
    ExperimentalPlacidusStatus.NO_REQUIRED_ROOTS: "R",
    ExperimentalPlacidusStatus.NO_ORDERED_SOLUTION: "O",
    ExperimentalPlacidusStatus.AMBIGUOUS_ORDERED_SOLUTION: "A",
}
_TIMESTAMPS_CACHE: list["TimestampMeta"] | None = None
...
def _timestamps() -> list[TimestampMeta]:
    global _TIMESTAMPS_CACHE
    if _TIMESTAMPS_CACHE is None:
        _TIMESTAMPS_CACHE = _build_timestamps()
...
def _process_latitude(latitude: float) -> dict[str, object]:
    ...
    return {
        "by_latitude_row": by_latitude_row,
        "first_success_payload": first_success_payload,
...
def main() -> None:
    ...
    worker_count = min(max((os.cpu_count() or 1) - 1, 1), 12)
    processed: list[dict[str, object]] = []
    with ProcessPoolExecutor(max_workers=worker_count) as executor:
        for result in executor.map(_process_latitude, latitudes, chunksize=4):
    ...
    with CALENDAR_CSV.open("w", newline="", encoding="utf-8") as calendar_handle:
        ...
        for result in processed:
            by_latitude_rows.append(result["by_latitude_row"])
            first_success_payload = result["first_success_payload"]
            if first_success_payload is not None:
                sample_first_success_by_latitude.append(first_success_payload)
            calendar_summaries.append(result["calendar_summary"])
            for day_row in result["day_rows"]:
                calendar_writer.writerow(day_row)
