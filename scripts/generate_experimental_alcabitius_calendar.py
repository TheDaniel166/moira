...
BY_LATITUDE_CSV = REPORTS_DIR / "experimental_alcabitius_greenwich_2000_2h_by_latitude.csv"
SUMMARY_JSON = REPORTS_DIR / "experimental_alcabitius_greenwich_2000_2h_summary.json"
CALENDAR_CSV = REPORTS_DIR / "experimental_alcabitius_greenwich_2000_2h_daily_calendar.csv"
CALENDAR_SUMMARY_JSON = REPORTS_DIR / "experimental_alcabitius_greenwich_2000_2h_daily_calendar_summary.json"
SLOT_LABELS = tuple(f"{hour:02d}" for hour in range(0, 24, 2))
STATUS_ORDER = (
    ExperimentalAlcabitiusStatus.UNIQUE_ORDERED_SOLUTION,
    ExperimentalAlcabitiusStatus.ASSEMBLY_FAILED,
    ExperimentalAlcabitiusStatus.UNORDERED_CUSP_CYCLE,
)
STATUS_CODE = {
    ExperimentalAlcabitiusStatus.UNIQUE_ORDERED_SOLUTION: "U",
    ExperimentalAlcabitiusStatus.ASSEMBLY_FAILED: "A",
    ExperimentalAlcabitiusStatus.UNORDERED_CUSP_CYCLE: "O",
}
...
def _evaluate(
    latitude: float,
    ts: TimestampMeta,
) -> tuple[ExperimentalAlcabitiusStatus, tuple[float, ...] | None]:
...
def main() -> None:
    ...
    with CALENDAR_CSV.open("w", newline="", encoding="utf-8") as calendar_handle:
        ...
        for latitude in latitudes:
            ...
            for ts in timestamps:
                ...
                succeeded = status == ExperimentalAlcabitiusStatus.UNIQUE_ORDERED_SOLUTION
                total_count += 1
    ...
    SUMMARY_JSON.write_text(
        json.dumps(
            {
                "sweep": {
                    "system": "Alcabitius experimental",
