...
BY_LATITUDE_CSV = REPORTS_DIR / "experimental_koch_greenwich_2000_2h_by_latitude.csv"
SUMMARY_JSON = REPORTS_DIR / "experimental_koch_greenwich_2000_2h_summary.json"
CALENDAR_CSV = REPORTS_DIR / "experimental_koch_greenwich_2000_2h_daily_calendar.csv"
CALENDAR_SUMMARY_JSON = REPORTS_DIR / "experimental_koch_greenwich_2000_2h_daily_calendar_summary.json"
SLOT_LABELS = tuple(f"{hour:02d}" for hour in range(0, 24, 2))
STATUS_ORDER = (
    ExperimentalKochStatus.UNIQUE_ORDERED_SOLUTION,
    ExperimentalKochStatus.HORIZON_BRANCH_SELECTION_FAILED,
    ExperimentalKochStatus.ASSEMBLY_FAILED,
    ExperimentalKochStatus.UNORDERED_CUSP_CYCLE,
    ExperimentalKochStatus.NO_VALID_SOLUTION,
)
STATUS_CODE = {
    ExperimentalKochStatus.UNIQUE_ORDERED_SOLUTION: "U",
    ExperimentalKochStatus.HORIZON_BRANCH_SELECTION_FAILED: "B",
    ExperimentalKochStatus.ASSEMBLY_FAILED: "A",
    ExperimentalKochStatus.UNORDERED_CUSP_CYCLE: "O",
    ExperimentalKochStatus.NO_VALID_SOLUTION: "N",
}
...
def _evaluate(
    latitude: float,
    ts: TimestampMeta,
) -> tuple[ExperimentalKochStatus, tuple[float, ...] | None]:
    ...
    with CALENDAR_CSV.open("w", newline="", encoding="utf-8") as calendar_handle:
        ...
        for latitude in latitudes:
            ...
            for ts in timestamps:
                ...
                succeeded = status == ExperimentalKochStatus.UNIQUE_ORDERED_SOLUTION
                total_count += 1
