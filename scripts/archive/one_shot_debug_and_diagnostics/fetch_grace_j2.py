"""
fetch_grace_j2.py — quarantined GRACE/GRACE-FO TN-14 research generator.
...
The generated series is *not* an admitted Moira runtime component. Correcting
the dimensional integration below does not establish the candidate
C20-to-inertia-to-LOD derivation, a cryosphere-only attribution, or an
uncertainty model. Running the archived generator therefore requires an
explicit acknowledgement flag; its output must not be consumed by
``delta_t_physical`` automatically.

Usage (research only):
    python scripts/archive/one_shot_debug_and_diagnostics/fetch_grace_j2.py \
...
def _integrate_lod_to_delta_t(
    ...
    Candidate cumulative ΔT(y) = Σ ΔLOD(t_i) × Δt_i / 1000,
    where ΔLOD is in milliseconds per day and Δt_i is in days.

    The historical implementation divided by 86400, even though multiplication
    by elapsed days had already performed the time integration. The only
    dimensional conversion still required is milliseconds to seconds (/1000).
    This numerical correction does not admit the upstream physical attribution.
...
def main() -> int:
    if "--quarantined-research-output" not in sys.argv[1:]:
        print(
            "Refusing to regenerate a quarantined research artifact without "
            "--quarantined-research-output. The corrected integration does not "
            "establish C20-to-LOD or cryosphere-only attribution."
        )
        return 2
    ...
    with _OUTPUT.open("w", encoding="utf-8") as fh:
        fh.write(
            "# QUARANTINED RESEARCH ARTIFACT - NOT RUNTIME-ADMITTED\n"
            "# Candidate GRACE/GRACE-FO contribution; attribution is unproved\n"
            "# Generated by scripts/archive/one_shot_debug_and_diagnostics/fetch_grace_j2.py\n"
            "# Corrected dimensional integration alone does not admit these values\n"
