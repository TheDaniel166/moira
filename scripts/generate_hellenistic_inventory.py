...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    Capability(
        "natal_twelfth_parts",
        "Natal twelfth-parts (dodekatemoria)",
        "admitted",
        "dedicated REST; optional profile overlay later",
        "moira.twelfth_parts",
        ("TwelfthPartPosition", "twelfth_part_of"),
        "source-owned golden plus REST parity",
        "Ptolemy Tetrabiblos I.22; ordinary 12× natal projection",
        "Not Vedic Dwadashamsa and not Sahl/Dorotheus electional clauses.",
        ("Hellenistic atoms",),
    ),
    Capability(
        "assemble_condition",
        "Score-free assemble-condition",
        "admitted",
        "dedicated REST; optional profile overlay later",
        "moira.hellenistic_relations",
        ("HellenisticAssembleCondition", "assemble_hellenistic_condition"),
        "testimony, overcoming, enclosure, adherence, and REST parity tests",
        "Antiochus via Porphyry; Valens on overcoming; existing superiority and besieging vessels",
        "No maltreatment score. Aktinobolia remains not_evaluable / doctrine_not_admitted.",
        ("Hellenistic atoms",),
    ),
    Capability(
        "supporting_exaltation_basis_lots",
        "Exaltation and Basis supporting lots",
        "admitted",
        "outside the four-lot partition",
        "moira.lots",
        ("HELLENISTIC_SUPPORTING_LOTS", "select_supporting_hellenistic_lots"),
        "supporting-lot helper tests",
        "Valens catalogue Exaltation and Basis (Valens)",
        "These lots are not added to HELLENISTIC_PROFILE_LOTS.",
    ),
    Capability(
        "zr_peak_grade",
        "ZR three-grade peak projection",
        "admitted_qualified",
        "revival display overlay",
        "moira.timelords",
        ("ZRPeakGrade", "zr_peak_grade"),
        "revival default tests",
        "modern revival display of Fortune-relative angularity",
        "Does not replace is_peak_period. Omitted ZR depth remains L1/L2.",
    ),
    Capability(
        "circumambulations",
        "Circumambulations through Egyptian bounds",
        "admitted_qualified",
        "REST-only; not a profile component",
        "moira.circumambulations",
        ("CircumambulationResult", "circumambulate"),
        "bound-walk and fail-closed time-key tests",
        "Egyptian bounds; Valens planetary periods as the only admitted year key",
        "Releaser is caller-named. Rising-time tables and equatorial/PD keys stay not_evaluable.",
        ("Hellenistic systems",),
    ),
    Capability(
        "valens_transmissions_graph",
        "Valens from-to transmission graph",
        "admitted",
        "REST-only; not a profile component",
        "moira.valens_transmissions",
        ("TransmissionGraph", "valens_transmission_graph"),
        "graph assembly and no-effect-field tests",
        "Valens IV period handoff as computational edges",
        "No effect prose, polarity, or scores. Closed interpretive exclusion remains closed.",
        ("Hellenistic systems",),
    ),
    Capability(
        "hellenistic_offices",
        "Fail-closed predominator and house-master hunt",
        "admitted_qualified",
        "REST-only; not a profile component",
        "moira.hellenistic_offices",
        ("HellenisticOfficeHunt", "hunt_hellenistic_offices"),
        "candidate preservation and no-hyleg import tests",
        "Hellenistic office names without a scored hybrid hunt",
        "No winner is selected. Bonatti hyleg and longevity.py are not imported.",
        ("Hellenistic systems",),
    ),
    Capability(
        "hermetic_catalog",
...
def render_api_inventory(app: Any | None = None) -> str:
    ...
    rows.extend(
        [
            ...
            "",
            "- Dedicated Hellenistic routes are the unified profile, the two "
            "whole-sign aspect routes, and the twelfth-parts / assemble-condition "
            "atom routes.",
