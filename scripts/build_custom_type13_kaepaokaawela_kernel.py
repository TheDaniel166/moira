...
def _verify_round_trip(path: Path, center: int, naif_id: int, epochs_jd: list[float], states: list[list[float]]) -> dict[str, float]:
...
def main() -> None:
    ...
    verification = _verify_round_trip(OUTPUT_BSP, TARGET["center"], TARGET["naif_id"], epochs_jd, states_km_s)
    payload = {
        "target": TARGET,
        "source": {
