# Morinus Primary Directions Truth Card
...
- on the explicit Morinian aspect branch, Moira now admits the circle-of-aspects
  ...
  - the motion coefficient `k` indicating approach to or departure from
    maximum latitude
  - explicit handed aspect angle
- the admitted Morinian aspect equations are:
  - `lambda' = asin(sin(delta_P) / sin(delta_max)) + k * aspect`
  - `AE = asin(tan(delta_P) / tan(delta_max))`
  - `AG = atan(cos(delta_max) * tan(lambda'))`
  - `delta = asin(sin(lambda') * sin(delta_max))`
  - `lambda = lambda_P + k * (AG - AE)`
...
- runtime-selectable on:
  - `in_mundo`
  - `in_zodiaco`
- active relation policy on the explicit aspect branch:
  - `conjunction`
  - `opposition`
  - `zodiacal_aspect`
- explicit Morinian aspect-plane projection is admitted for zodiacal
  aspect-point promissors when `MorinusAspectContext` is supplied by the
  service layer
...
- on current evidence, Moira does **not** treat conjunction-style Morinus as a
  ...
  source-safe distinct conjunction law as the present doctrinal limit
