...
def test_asteroid_fixture_cases_match_under_native_reader_pool(
    case: dict,
    small_body_reader_pool: KernelPool,
) -> None:
    result = asteroid_at(
        case["body"],
        case["jd_ut"],
...
def test_small_body_public_positions_remain_smooth_over_one_second(
    ...
    if body_name in COMET_NAIF:
        before = comet_at(
            body_name,
            jd_ut - _ONE_SECOND_JD,
            reader=small_body_reader_pool,
        )
        current = comet_at(body_name, jd_ut, reader=small_body_reader_pool)
        after = comet_at(
            body_name,
            jd_ut + _ONE_SECOND_JD,
            reader=small_body_reader_pool,
        ...
        current = asteroid_at(body_name, jd_ut, reader=small_body_reader_pool)
        after = asteroid_at(
            body_name,
            jd_ut + _ONE_SECOND_JD,
            reader=small_body_reader_pool,
...
def test_small_body_kernels_fail_cleanly_outside_body_coverage(
    ...
    coverage = small_body_reader_pool.coverage()
    ...
    for naif_id in representative_targets:
        ...
        with pytest.raises(OutOfRangeError, match="No kernel covers"):
            small_body_reader_pool.position(center, naif_id, start_jd - 1.0)
        with pytest.raises(OutOfRangeError, match="No kernel covers"):
            small_body_reader_pool.position(center, naif_id, end_jd + 1.0)
