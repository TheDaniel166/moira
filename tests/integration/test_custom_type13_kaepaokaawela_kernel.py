...
def test_custom_kaepaokaawela_type13_kernel_round_trips_through_public_asteroid_api() -> None:
    if not _META.exists():
        pytest.skip("kaepaokaawela type13 metadata artifact is missing")

    payload = json.loads(_META.read_text(encoding="utf-8"))
    kernel_path = _ROOT / payload["output_bsp"]
    if not kernel_path.exists():
        pytest.skip("kaepaokaawela type13 BSP artifact is missing")
...
def test_custom_kaepaokaawela_type13_kernel_matches_live_horizons_observer_product() -> None:
    if not _META.exists():
        pytest.skip("kaepaokaawela type13 metadata artifact is missing")
    ...
    try:
        pool = KernelPool(readers)
        # Ka`epaoka`awela's orbital period (~11.65y) is slow relative to
        # Toutatis's, but sample three dates across the kernel's coverage
        # rather than one, since it is a retrograde co-orbital and its
        # interpolation behavior is not otherwise exercised elsewhere.
        for jd_ut in (
