"""
Test of the critical latitude: house fallback above 90° − obliquity.

This suite verifies that semi-arc systems (Placidus, Koch) fall
back to Porphyry above the critical latitude, which is 90° − obliquity
(≈ 66.56° at J2000).  This threshold is not a fixed constant — it is derived
from the chart's actual obliquity at call time.

The old threshold of 75° was incorrect: it silently returned geometrically
invalid cusp sets from ≈66.6° to 74.9°.
"""

from datetime import datetime, timezone
import pytest
from moira import Moira
from moira.constants import HouseSystem
from moira.houses import HousePolicy

def test_north_pole_fallback_triggers_porphyry(moira_engine):
    """At the North Pole (90°N), Placidus must fall back to Porphyry."""
    dt = datetime(2000, 3, 20, 7, 35, tzinfo=timezone.utc)

    polar_placidus = moira_engine.houses(dt, latitude=90.0, longitude=0.0, system=HouseSystem.PLACIDUS)
    polar_porphyry = moira_engine.houses(dt, latitude=90.0, longitude=0.0, system=HouseSystem.PORPHYRY)

    assert polar_placidus.system == HouseSystem.PLACIDUS
    for i in range(12):
        assert polar_placidus.cusps[i] == pytest.approx(polar_porphyry.cusps[i], abs=1e-8)
    assert polar_placidus.asc == pytest.approx(polar_porphyry.asc, abs=1e-8)
    assert polar_placidus.mc  == pytest.approx(polar_porphyry.mc,  abs=1e-8)

def test_arctic_threshold_boundary_behavior(moira_engine):
    """
    The critical latitude is 90° − obliquity (≈ 66.56° at J2000).
    60° is safely below it (no fallback for Placidus).
    70° is above it; for this particular ARMC the integrated Placidus branch
    search finds no unique ordered solution, so default policy falls back.
    (Other ARMCs at 70N+ can yield valid Placidus under default policy.)
    """
    dt = datetime(2000, 3, 20, 7, 35, tzinfo=timezone.utc)

    # 60°N: well below critical latitude — Placidus must remain Placidus
    sub = moira_engine.houses(dt, latitude=60.0, longitude=0.0, system=HouseSystem.PLACIDUS)
    assert sub.effective_system == HouseSystem.PLACIDUS
    assert sub.fallback is False

    # 70°N (this ARMC): above critical; no unique ordered Placidus solution
    # for this position, so falls back to Porphyry under default policy.
    supra       = moira_engine.houses(dt, latitude=70.0, longitude=0.0, system=HouseSystem.PLACIDUS)
    supra_porph = moira_engine.houses(dt, latitude=70.0, longitude=0.0, system=HouseSystem.PORPHYRY)
    assert supra.effective_system == HouseSystem.PORPHYRY
    assert supra.fallback is True
    for i in range(12):
        assert supra.cusps[i] == pytest.approx(supra_porph.cusps[i], abs=1e-8)

def test_south_pole_fallback_triggers_porphyry(moira_engine):
    """South Pole (−90°S) must also fall back symmetrically."""
    dt = datetime(2000, 3, 20, 7, 35, tzinfo=timezone.utc)

    polar_placidus = moira_engine.houses(dt, latitude=-90.0, longitude=0.0, system=HouseSystem.PLACIDUS)
    polar_porphyry = moira_engine.houses(dt, latitude=-90.0, longitude=0.0, system=HouseSystem.PORPHYRY)

    for i in range(12):
        assert polar_placidus.cusps[i] == pytest.approx(polar_porphyry.cusps[i], abs=1e-8)


def test_moira_houses_propagates_explicit_strict_policy(moira_engine):
    dt = datetime(2000, 3, 20, 7, 35, tzinfo=timezone.utc)

    with pytest.raises(ValueError, match="critical latitude"):
        moira_engine.houses(
            dt,
            latitude=90.0,
            longitude=0.0,
            system=HouseSystem.PLACIDUS,
            policy=HousePolicy.strict(),
        )

