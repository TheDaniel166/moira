"""Explicit-kernel manual stress audit; intentionally outside pytest discovery."""
...
def stress_test_resource_binding(reader: SpkReader) -> bool:
    ...
    if (p1.x, p1.y, p1.z) != (p2.x, p2.y, p2.z):
        raise AssertionError("Independent readers produced different positions")

    # Close one, ensure other still works
    reader2.close()
    p3 = planet_at(body, jd, reader=reader, frame='cartesian', apparent=False)
    if (p1.x, p1.y, p1.z) != (p3.x, p3.y, p3.z):
        raise AssertionError("Closing the second reader changed the first reader")
