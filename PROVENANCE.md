...
heliocentric `VECTORS` states and materialized as Moira Type-13 shards. The
canonical identity artifact in `moira/data/asteroid_catalog_naif.json` is bound
to finalized release `2026.07.27.1` by
`moira/data/asteroid_catalog_naif.metadata.json`, including the exact release
manifest, admitted-target, unified-ledger, and artifact SHA-256 values. The BSP
files and their release manifest are separately acquired runtime resources.
At runtime, the resolved external manifest and its available shards govern
position capability and coverage; a bundled canonical identity alone does not.
