...
The packaged `moira-deep-sky` catalog version `2026.09.07.1` is a
Moira-curated, non-interpretive selection prompted by
[GitHub Discussion #19](https://github.com/TheDaniel166/moira/discussions/19).
It contains 60 coordinate anchors: 15 galaxies, 15 nebulae, 15 star clusters,
5 compact objects, 5 stellar remnants, and 5 confirmed exoplanet host stars.
Selection and classification are Moira-owned; they are not represented as a
complete astronomical catalog or as doctrine about astrological significance.

SIMBAD is used as a resolution and source-compilation service, not described
as the author of the selection.  The refresh tool resolves each admitted
designation, captures the returned SIMBAD main identity, ICRS/J2000 right
ascension and declination, object type, coordinate-quality code, coordinate
bibcode when supplied, and proper-motion components.  The source snapshot,
query fields, SIMBAD release, requested acknowledgement, ODbL 1.0 data license,
and Wenger et al. reference are retained in
`moira/data/deep_sky_catalog.metadata.json`; the packaged artifact is bound by
SHA-256 `5ded4bec9fd8f4b1e3855f21a993fbe040d2b74bf04ffdda74e6103b974bf720`.
The adjacent `deep_sky_NOTICE.txt` keeps that data license and attribution
visible in built wheels.

Runtime projection treats an extended-object coordinate as a catalog center,
not a physical point mass.  Galaxy, nebula, remnant, quasar, and Galactic-center
anchors remain fixed at their released J2000 direction.  Only the explicitly
admitted star-cluster centers and stellar compact systems use SIMBAD proper
motion before Moira's existing IAU 2006 precession, IAU 2000A nutation, and
true-obliquity transformation to true ecliptic of date.  The five host stars
are independently checked against NASA Exoplanet Archive `pscomppars` and
delegate runtime positions to Moira's sovereign star registry, preserving its
proper-motion, parallax, and radial-velocity model.

Solar-System bodies are outside this catalog.  In particular, a J2000
coordinate cannot substitute for the time-dependent ephemeris of a planetary
satellite, comet, asteroid, or interstellar visitor.  Halley and
67P/Churyumov-Gerasimenko already belong to the numbered-periodic-comet release;
Hale-Bopp, NEOWISE, West, and 1I/ʻOumuamua require separately admitted Horizons
trajectories, while non-lunar planetary satellites require their applicable
JPL satellite SPKs and coverage receipts.

### Typed cosmic reference registry

The `moira-cosmic-references` registry version `2026.09.08.1` contains 12
fixed ICRS/J2000 directions and deliberately separates three kinds of
reference: 2 physical objects, 7 coordinate landmarks, and 3 declared proxy
references. Runtime conversion uses Moira's existing ICRF/J2000 to true
ecliptic-of-date path; these records do not add proper motion, distance, mass,
or gravitational-center claims.

The galactic origin, anti-center, and poles use the ICRS realization described
by [Liu, Zhu & Zhang (2011, A&A 526 A16)](https://www.aanda.org/articles/aa/abs/2011/02/aa14961-10/aa14961-10.html).
The formal supergalactic longitude origin and poles use the de Vaucouleurs
frame coordinates tabulated by [Chandra CIAO](https://cxc.cfa.harvard.edu/ciao/ahelp/prop-coords.html)
and are oracle-checked against Astropy's `Supergalactic` frame. The formal
supergalactic origin is a coordinate-frame landmark and is not M87.

Sagittarius A* and M87 reuse the coordinate receipts in the packaged deep-sky
catalog. The historical astrological `Super-Galactic Center` remains available
as the separately named `Virgo/M87 Astrological SGC` proxy anchored on M87;
[NED's Virgo structure review](https://ned.ipac.caltech.edu/level5/Binggeli/Bin2.html)
is retained to prevent that convention from being presented as a unique Virgo
Cluster or Local Supercluster barycenter.

The Great Attractor entry is explicitly the SIMBAD catalog center of the Norma
Cluster (`ACO 3627`; ICRS RA `243.59375` degrees, Dec
`-60.86861111111111` degrees), informed by Woudt & Kraan-Korteweg's
potential-well [assessment](https://arxiv.org/abs/astro-ph/0006126). The Shapley
entry is explicitly the SIMBAD catalog
center of `ACO 3558` (ICRS RA `202.0135` degrees, Dec `-31.5265` degrees)
within a multicluster core, informed by [Akimoto et al.](https://arxiv.org/abs/astro-ph/0310802).
Both SIMBAD lookups were
captured on 2026-09-08 against SIMBAD4 release `1.8 - 2026-07`; the SIMBAD
acknowledgement and ODbL declaration in `moira/data/deep_sky_NOTICE.txt` apply.

No Local Group barycenter is published. Such a point depends on a declared
membership, mass model, and epoch; admitting one fixed coordinate without that
model would manufacture precision rather than preserve astronomical truth.
