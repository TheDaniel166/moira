...
| PP-10 | Public `planet_at(...)` canonical product | [moira/planets.py](/c:/Users/nilad/OneDrive/Desktop/Moira%20C++/moira/planets.py:610) | Python over the admitted shared planetary substrate | Native-supported reader, route-cache, and Earth/geocentric substrate now participate in normal `planet_at(...)` execution without caller patching | Partial | The checked artifact is still only weakly positive overall (~`1.02x` median on the combined cold-dominated total), while warm steady-state remains slightly negative (~`0.98x` median), so no stronger closure claim is justified yet | Existing public semantics and Horizons-backed apparent tests must remain exact if deeper routing changes | `native_phase2_planet_at.json` or successor artifact on supported bodies and representative dates | Public call must continue to use the admitted substrate in ordinary `.venv` execution with no script-only setup | PP-09 |
| PP-11 | Public `all_planets_at(...)` chart workload | [moira/planets.py](/c:/Users/nilad/OneDrive/Desktop/Moira%20C++/moira/planets.py:1000) | Python over the admitted shared planetary substrate | Native-supported reader plus a real admitted native planetary evaluator now participate in normal `all_planets_at(...)` execution without caller patching | Partial | The checked artifact now shows a real warm-reader gain for the admitted surface (~`1.63x` median), but cold-reader total remains near parity because evaluator setup cost still dominates whole-run totals | Existing chart-level semantics and ordering must remain stable under the routed substrate | `native_phase2_all_planets.json` or successor artifact for repeated chart-style workloads | Normal chart assembly must hit the same routed planetary substrate in `.venv` with no script-only setup | PP-10 |
