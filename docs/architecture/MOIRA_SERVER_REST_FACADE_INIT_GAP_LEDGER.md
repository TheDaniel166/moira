...
| Derived-chart aspects | `find_aspects`, `aspects_from_longitudes`, `Moira.aspects_from_longitudes` | `/v1/aspects/from-longitudes` exposes arbitrary supplied positions; `/v1/composite/chart` and `/v1/davison/chart` embed the same analysis under `aspects` | `admitted_positions_in_and_relationship_composed` | Reuses the engine's aspect definitions and `AspectData`; does not invent a moment, speeds, applying/separating state, or a second orb table. |
