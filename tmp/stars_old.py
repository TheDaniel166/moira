"""
Star Catalogue Oracle ΓÇö moira/stars.py
...
Owns:
    ...
    - star_at()            ΓÇö named lookup with optional Gaia enrichment
    - stars_near()         ΓÇö positional search across both catalogs
    - stars_by_magnitude() ΓÇö magnitude-filtered list across both catalogs
    - list_named_stars() / find_named_stars() ΓÇö catalog introspection
Delegates:
...
Public surface / exports:
    FixedStar             ΓÇö unified result dataclass
    star_at()             ΓÇö named star lookup
    stars_near()          ΓÇö positional search
    stars_by_magnitude()  ΓÇö magnitude-filtered list
    list_named_stars()    ΓÇö all traditional names in Hipparcos catalog
    find_named_stars()    ΓÇö name-fragment search

Merge strategy
--------------
  ΓÇó Named lookup  : sefstars is primary (has the name). If the same star is
                    also in Gaia (matched within 0.05┬░ on sky), Gaia fields
                    (parallax, distance, BPΓêÆRP, teff, quality) are grafted on.
  ΓÇó Positional    : both catalogs are queried; stars within 0.05┬░ of each
                    other are deduplicated (sefstars preferred when named).
  ΓÇó Magnitude-cut : all sefstars entries pass (they are all bright/named);
                    Gaia entries are filtered to G ΓëÑ 3 to exclude saturated
...
_MATCH_RADIUS_SKY = 0.05 # degrees on sky for sefstars Γåö Gaia cross-match
...
class FixedStar:
    """
    RITE: The Unified Witness ΓÇö the merged position of a fixed star from
    ...
    RITE OF PURPOSE:
        ...
        It unifies two fundamentally different catalog backends ΓÇö the named,
        bright-star Hipparcos/sefstars catalog and the deep, photometric Gaia
        DR3 catalog ΓÇö into one coherent type.  Callers need not know which
    ...
    [MACHINE_CONTRACT v1]
    {
        ...
        "failures": "None ΓÇö construction only fails if caller passes wrong types.",
        "succession": {
            "stance": "terminal",
            "extension_points": []
        },
...
def _find_gaia_match(
    ...
    # computing full sky separations ΓÇö avoids scanning all 155K records.
...
def stars_near(
    ...
    Queries both Hipparcos (named stars) and Gaia (G = 3ΓÇô10) and returns
...
def stars_by_magnitude(
    ...
    G Γëê 3 (which Gaia saturates on), Gaia handles G = 3ΓÇô10.
